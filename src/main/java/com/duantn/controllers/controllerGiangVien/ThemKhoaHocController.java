...
public class ThemKhoaHocController {
    ...
    public String showCourseForm(
            @RequestParam(value = "khoahocId", required = false) Integer khoahocId, Model model) {
        KhoaHoc khoaHoc;
        if (khoahocId != null) {
    ...
    public String saveBasicInfo(@ModelAttribute("course") KhoaHoc formCourse, BindingResult result,
            Model model, @RequestParam(value = "file", required = false) MultipartFile file,
            @RequestParam("danhMuc.danhmucId") Integer danhMucId, Principal principal)
            throws IOException {
        ...
        if (formCourse.getTenKhoaHoc() == null || formCourse.getTenKhoaHoc().trim().isEmpty()) {
            ...
            result.rejectValue("tenKhoaHoc", "error.course",
                    "Tên khóa học phải có ít nhất 3 ký tự");
            hasError = true;
        } else if (formCourse.getTenKhoaHoc().trim().length() > 250) {
            result.rejectValue("tenKhoaHoc", "error.course",
                    "Tên khóa học không được vượt quá 250 ký tự");
            hasError = true;
        } else if (!formCourse.getTenKhoaHoc().matches("^[\\p{L}0-9\\s.,?!():\"'-]+$")) {
            result.rejectValue("tenKhoaHoc", "error.course",
    ...
    public String savePrice(@ModelAttribute("course") KhoaHoc khoahocUpdate, BindingResult result,
        ...
        if (ptg != null) {
            if (ptg < 0 || ptg > 100) {
                result.rejectValue("phanTramGiam", "error.course",
        ...
        if (khoahoc.getGiagoc() != null && khoahoc.getPhanTramGiam() != null
                && khoahoc.getPhanTramGiam() > 0) {
            BigDecimal giam =
                    khoahoc.getGiagoc().multiply(BigDecimal.valueOf(khoahoc.getPhanTramGiam()))
